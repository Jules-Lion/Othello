...
num_moves = size(moves_list);

if (depth == 0 || num_moves(1) == 0)                     % Wenn die Suchtiefe gleich 0 ist und kein Spielzug fuer den Spieler vorhanden ist, dann Bewerte
        max_v = evaluation(board, color);                   % Funktion muss noch geschrieben werden
else
   ...
   while (num_moves(1))
    ...
    % Zeile
    new_board = apply_move(board, color, moves_list(num_moves(1),:));                                        % muss noch geschrieben werden
    ...
    [value, best_move_max] = min (-color, depth-1, new_board);
    ...
    % hier wird der Knoten mit dem groessten Nutzwert bestimmt
        if (value > max_value)
            ...
            best_move_max = moves_list(num_moves(1),:);            % speicher den besten Spielzug
            b = apply_move(board, color, best_move_max);
        end
    
    % Anzahl der Spielzuege um eins reduzieren
    num_moves(1) = num_moves(1) - 1;
...
function [min_v, best_move_max]  = min(color, depth, board)
...
if (depth == 0 || num_moves(1) == 0)                     % Wenn die Suchtiefe gleich 0 ist und kein Spielzug fuer den Spieler vorhanden ist, dann Bewerte
    ...
    while (num_moves(1))
        ...
        % 
        new_board = apply_move(board, color, moves_list(num_moves(1),:));
        %
        [value, best_move_max] = MiniMax(-color, depth-1, new_board);
        % 
        %board = back_move(board, moves_list(num_moves,:));
    
        if (value < min_value)
            min_value = value;                              % weise max_value den groesseren Wert zu
            best_move_max = moves_list(num_moves(1),:);        % best_move_max entweder gloabl definieren in der Hauptfunktion oder in der Funktion selbst als rueckgabewert
            b = apply_move(board, color, best_move_max);
